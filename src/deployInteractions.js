...
const fetchInteractions = async (client, interactions) => {
    try {
        const interactionsInDb = await client.prisma.interaction.findMany();

        interactions.forEach(async (interaction) => {
            await client.prisma.interaction.upsert({
                where: {
            ...
            interaction.data.options.forEach(async (option) => {
                await client.prisma.interactionOption.upsert({
                    where: {
        ...
        interactionsInDb.forEach(async (interactionInDb) => {
            if (!interactions.find((interaction) => interaction.data.name === interactionInDb.name)) {
                await client.prisma.interaction.delete({
                    where: {
...
const checkIfAnyChanges = async (client) => {
    const interactionsInDb = await client.prisma.interaction.findMany({ include: { options: true } });
...
const deployInteractions = async (client) => {
    ...
    try {
        ...
        await fetchInteractions(client, client.interactions);
